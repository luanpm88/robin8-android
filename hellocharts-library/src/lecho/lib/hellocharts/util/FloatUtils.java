...
public class FloatUtils {
    ...
    public static double nextDown(double d) {
        if (Double.isNaN(d) || d == Double.NEGATIVE_INFINITY) {
            ...
            if (d == 0.0f) {
                ...
                return Double.longBitsToDouble(Double.doubleToRawLongBits(d) + ((d > 0.0f) ? -1 :
                        +1));
            }
        }
    }
     ...
     * [endIndex] - [return value] and ends at [endIndex]. It's up to you to check indexes
     * correctness.
     ...
     */
    public static int formatFloat(final char[] formattedValue, float value, int endIndex, int
            digits, char separator) {
    ...
    public static void computeAutoGeneratedAxisValues(float start, float stop, int steps,
        ...
        if (range <= 0) {
            outValues.values = new float[]{1, 2, 3, 4,5,6,7,8};
            outValues.valuesNumber = 8;
            outValues.decimals=1;
            return;
        }

        double rawInterval = range / steps;//步长
        double interval = roundToOneSignificantFigure(rawInterval);//求整
        ...
        outValues.valuesNumber = valuesNum;//总步长
        ...
        for (intervalValue = first, valueIndex = 0; valueIndex < valuesNum; intervalValue +=
                interval, ++valueIndex) {
