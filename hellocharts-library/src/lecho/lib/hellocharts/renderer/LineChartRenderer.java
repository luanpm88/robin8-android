...
public class LineChartRenderer extends AbstractChartRenderer {
    ...
    private static final int DEFAULT_TOUCH_TOLERANCE_MARGIN_DP = 10;//左右两边的margin
    ...
    public LineChartRenderer(Context context, Chart chart, LineChartDataProvider dataProvider) {
        ...
        touchToleranceMargin = ChartUtils.dp2px(density, 4);//触摸后放大的半径
    ...
    public void onChartSizeChanged() {
        ...
        if (computator.getChartWidth() > 0 && computator.getChartHeight() > 0) {
            softwareBitmap = Bitmap.createBitmap(computator.getChartWidth(), computator
                            .getChartHeight(),
    ...
    public void draw(Canvas canvas) {
        ...
        // softwareBitmap can be null if chart is rendered in layout editor. In that case use
        // default canvas and not
        // softwareCanvas.
    ...
    /**
     * Draws lines, uses path for drawing filled area on software canvas. Line is drawn with
    ...
    private void drawPoints(Canvas canvas, Line line, int lineIndex, int mode) {
        ...
        for (PointValue pointValue : line.getValues()) {
            ...
            if (computator.isWithinContentRect(rawX, rawY, checkPrecision)) {
                // Draw points only if they are within contentRectMinusAllMargins, using
                // contentRectMinusAllMargins
                // instead of viewport to avoid some
                // float rounding problems.
                if (MODE_DRAW == mode) {
                    //自己加的
                    if (MODE_DRAW == pointValue.getMode()) {
                        drawPoint(canvas, line, pointValue, rawX, rawY, pointRadius, 0);
                    } else if (MODE_HIGHLIGHT == pointValue.getMode()) {
                        drawPoint(canvas, line, pointValue, rawX, rawY, pointRadius +
                                touchToleranceMargin, line.getPointHilightColor());
                    }
                    if (line.hasLabels()) {//画点上的标签
    ...
    private void drawPoint(Canvas canvas, Line line, PointValue pointValue, float rawX, float rawY,
                           float pointRadius, int color) {
        int normalColor = pointPaint.getColor();
        if (color != 0) {
    ...
    private void highlightPoint(Canvas canvas, Line line, PointValue pointValue, float rawX,
                                float rawY, int lineIndex,
                                int valueIndex) {
        if (selectedValue.getFirstIndex() == lineIndex && selectedValue.getSecondIndex() ==
                valueIndex) {
            int pointRadius = ChartUtils.dp2px(density, line.getPointRadius());
            pointPaint.setColor(line.getDarkenColor());
            drawPoint(canvas, line, pointValue, rawX, rawY, pointRadius + touchToleranceMargin,0);
    ...
    private void drawLabel(Canvas canvas, Line line, PointValue pointValue, float rawX, float
            rawY, float offset) {
        ...
        final float labelWidth = labelPaint.measureText(labelBuffer, labelBuffer.length -
                numChars, numChars);
    ...
    private void drawArea(Canvas canvas, Line line) {
        ...
        final Rect contentRect = computator.getContentRectMinusAllMargins();
        final float baseRawValue = Math.min(contentRect.bottom, Math.max(computator.computeRawY
                        (baseValue),
                contentRect.top));
        //That checks works only if the last point is the right most one.
        final float left = Math.max(computator.computeRawX(line.getValues().get(0).getX()),
                contentRect.left);
        final float right = Math.min(computator.computeRawX(line.getValues().get(lineSize - 1)
                        .getX()),
                contentRect.right);
