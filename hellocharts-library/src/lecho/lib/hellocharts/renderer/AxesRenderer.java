...
/**
 * Default axes renderer. Can draw maximum four axes - two horizontal(top/bottom) and two
 * vertical(left/right).
 */
public class AxesRenderer {
    ...
    /**
     * Used to measure label width. If label has mas 5 characters only 5 first characters of this
     * array are used to
     * measure text width.
     */
    ...
    private FontMetricsInt[] fontMetricsTab = new FontMetricsInt[]{new FontMetricsInt(), new
            FontMetricsInt(),
    ...
    /**
     * Holds auto-generated values that should be drawn, i.e if axis is inside not all
     * auto-generated values should be
    ...
    private void initAxisDimensionForTiltedLabels(int position) {
        ...
        labelDimensionForStepsTab[position] = Math.round(labelDimensionForMarginsTab[position] *
                0.75f);
    }
    ...
    private void intiTiltedLabelsTranslation(Axis axis, int position) {
        ...
        if (axis.isInside()) {
            if (LEFT == position) {
                ...
                dy = (pythagoreanFromAscent + pythagoreanFromLabelWidth / 2) -
    ...
    private void initAxisMeasurements(Axis axis, int position) {
        if (LEFT == position) {
            if (axis.isInside()) {
                labelBaselineTab[position] = computator.getContentRectMinusAllMargins().left +
                        axisMargin;
                nameBaselineTab[position] = computator.getContentRectMinusAxesMargins().left -
                        axisMargin
                        - labelTextDescentTab[position];
            } else {
                labelBaselineTab[position] = computator.getContentRectMinusAxesMargins().left -
                        axisMargin;
            ...
            if (axis.isInside()) {
                labelBaselineTab[position] = computator.getContentRectMinusAllMargins().right -
                        axisMargin;
                nameBaselineTab[position] = computator.getContentRectMinusAxesMargins().right +
                        axisMargin
                        + labelTextAscentTab[position];
            } else {
                labelBaselineTab[position] = computator.getContentRectMinusAxesMargins().right +
                        axisMargin;
                nameBaselineTab[position] = labelBaselineTab[position] + axisMargin
            ...
            if (axis.isInside()) {
                labelBaselineTab[position] = computator.getContentRectMinusAllMargins().bottom -
                        axisMargin
                        - labelTextDescentTab[position];
                nameBaselineTab[position] = computator.getContentRectMinusAxesMargins().bottom +
                        axisMargin
                        + labelTextAscentTab[position];
            } else {
                labelBaselineTab[position] = computator.getContentRectMinusAxesMargins().bottom +
                        axisMargin
                        + labelTextAscentTab[position];
                nameBaselineTab[position] = labelBaselineTab[position] + axisMargin +
            ...
            if (axis.isInside()) {
                labelBaselineTab[position] = computator.getContentRectMinusAllMargins().top +
                        axisMargin
                        + labelTextAscentTab[position];
                nameBaselineTab[position] = computator.getContentRectMinusAxesMargins().top -
                        axisMargin
                        - labelTextDescentTab[position];
            } else {
                labelBaselineTab[position] = computator.getContentRectMinusAxesMargins().top -
                        axisMargin
                        - labelTextDescentTab[position];
                nameBaselineTab[position] = labelBaselineTab[position] - axisMargin -
                        labelDimensionForMarginsTab[position];
    ...
    private void prepareCustomAxis(Axis axis, int position) {
        ...
        int module = (int) Math.max(1,
                Math.ceil((axis.getValues().size() * labelDimensionForStepsTab[position] *1.5) /
                        scale));
        //Reinitialize tab to hold lines coordinates.
        if (axis.hasLines() && (linesDrawBufferTab[position].length < axis.getValues().size() *
                4)) {
        ...
        for (AxisValue axisValue : axis.getValues()) {
            ...
            if (value >= viewportMin && value <= viewportMax) {
                // Draw axis values that have 0 module value, this will hide some labels if there
                ...
                if (0 == valueIndex % module) {
                    ...
                    if (checkRawValue(contentRect, rawValue, axis.isInside(), position,
                            isAxisVertical)) {
    ...
    private void prepareAutoGeneratedAxis(Axis axis, int position) {
        ...
        //Reinitialize tab to hold lines coordinates.
        if (axis.hasLines()
                && (linesDrawBufferTab[position].length < autoValuesBufferTab[position]
                .valuesNumber * 4)) {
            linesDrawBufferTab[position] = new float[autoValuesBufferTab[position].valuesNumber *
                    4];
        }
        ...
        for (int i = 0; i < autoValuesBufferTab[position].valuesNumber; ++i) {
            ...
            if (checkRawValue(contentRect, rawValue, axis.isInside(), position, isAxisVertical)) {
                rawValuesTab[position][valueToDrawIndex] = rawValue;
                autoValuesToDrawTab[position][valueToDrawIndex] = autoValuesBufferTab[position]
                        .values[i];
                ++valueToDrawIndex;
            }
        }
    ...
    private boolean checkRawValue(Rect rect, float rawValue, boolean axisInside, int position,
                                  boolean isVertical) {
        if (axisInside) {
            if (isVertical) {
    ...
    private void drawAxisLines(Canvas canvas, Axis axis, int position) {
        ...
        if (axis.hasSeparationLine()) {//画轴线
            canvas.drawLine(separationX1, separationY1, separationX2+30, separationY2,
        ...
        if (axis.hasLines()) {
            ...
            if (axis.isDrawVirtualLine()) {//画虚线
                Paint paint = new Paint();
                paint.setStyle(Paint.Style.STROKE);
                paint.setColor(Color.DKGRAY);
                if (linesDrawBufferTab.length >= 4) {
                    if (valueToDrawIndex == 0) {
                        Path path = new Path();
                        path.moveTo(linesDrawBufferTab[position][0],
                                linesDrawBufferTab[position][1]);
                        path.lineTo(linesDrawBufferTab[position][2],
                                linesDrawBufferTab[position][3]);
                        canvas.drawPath(path, paint);
                    }
                    for (int i = 1; i < valueToDrawIndex; i++) {
                        Path path = new Path();
                        path.moveTo(linesDrawBufferTab[position][i * 4 + 0],
                                linesDrawBufferTab[position][i * 4 + 1]);
                        path.lineTo(linesDrawBufferTab[position][i * 4 + 2],
                                linesDrawBufferTab[position][i * 4 + 3]);
                ...
                canvas.drawLines(linesDrawBufferTab[position], 0, valueToDrawIndex * 4,
    ...
    private void drawAxisLabelsAndName(Canvas canvas, Axis axis, int position) {
        ...
        for (int valueToDrawIndex = 0; valueToDrawIndex < valuesToDrawNumTab[position];
             ++valueToDrawIndex) {
            int charsNumber = 0;
            if (axis.isAutoGenerated()) {
                final float value = autoValuesToDrawTab[position][valueToDrawIndex];
                charsNumber = axis.getFormatter().formatValueForAutoGeneratedAxis(labelBuffer,
                        value,
            ...
            if (isAxisVertical) {
                labelY = rawValuesTab[position][valueToDrawIndex]+ChartUtils.dp2px(density,5);
            } else {
            ...
            if (axis.hasTiltedLabels()) {
                canvas.save();
                canvas.translate(tiltedLabelXTranslation[position],
                        tiltedLabelYTranslation[position]);
