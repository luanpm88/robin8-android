package com.robin8.rb.ui.module.mine.fragmennt;
...
import com.robin8.rb.ui.module.create.protocol.MyCreateFragmentProtocol;
import com.robin8.rb.ui.module.mine.protocol.MyPaticipateFragmentProtocol;
...
public class CampaignContentFragment extends BaseFragment {
    ...
    private void loadData() {
        switch (mPageName){
            case StatisticsAgency.MY_TASK:
                if (mUrl.contains("tenders")){
                    LogUtil.LogShitou("这里是大V",""+mUrl);
                    mProtocol = new MyPaticipateFragmentProtocol(mActivity,mData,mLLNodata,1);
                }else {
                    LogUtil.LogShitou("这里是小V",""+mUrl);
    ...
    public void getDataFromNet(int currentState) {
        ...
        mBasePresenter.getDataFromServer(true, HttpRequest.GET, mUrl, mRequestParams, new RequestCallback() {
            ...
            public void onResponse(String response) {
                LogUtil.LogShitou("大 v活动数据"+mUrl,response);
                if (mXRefreshView != null) {
                    mXRefreshView.stopRefresh();
                    mXRefreshView.stopLoadMore();
                }
